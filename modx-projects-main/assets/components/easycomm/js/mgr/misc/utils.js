easyComm.combo.ThreadResource = function(config) {
    ...
    Ext.applyIf(config,{
        ...
        ,baseParams: {
            action: 'System\\Element\\Resource\\GetList'
            ,combo: 1
            ,id: config.value
        }
        ,pageSize: 20
...
easyComm.combo.ReplyTemplate = function(config) {
    ...
    Ext.applyIf(config,{
        ...
        ,url: easyComm.config.connector_url
        ,baseParams: {
            action: 'ReplyTemplate\\GetComboList'
            ,combo: 1
            ,id: config.value
...
easyComm.utils.renderFilePreview = function(val, cell, row) {
    return val ?
        '<a target="_blank" href="' + val + '"><img src="'+MODx.config.connectors_url+'system/phpthumb.php?h=50&w=150&far=0&src='+val + '" /></a>'
        :
        '';
};

easyComm.utils.renderFileName = function(val, cell, row) {
    if(!val) {
        return '';
    }
    return '<a target="_blank" href="' + row.data.url + '" title="' + val + '">' + val + '</a>';
};
