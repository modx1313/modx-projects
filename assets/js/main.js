...
initPriceFilter();
initCatalogShipsFilter();
checkCookies();
syncCatalogFiltersDrawerState();
...
function isCatalogFiltersMobileLayout() {
  return window.innerWidth <= 768;
}

function openCatalogFiltersDrawer() {
  let drawer = document.querySelector('.js-catalog-filters-drawer');
  if (!drawer || !isCatalogFiltersMobileLayout()) {
    return false;
  }
...
function closeCatalogFiltersDrawer() {
  let drawer = document.querySelector('.js-catalog-filters-drawer');
  if (!drawer) {
    return false;
  }
...
function applyCatalogFiltersFromDrawer() {
  let priceFilterElement = document.querySelector('[data-filter-name="price"]');

  if (priceFilterElement) {
    let minInput = priceFilterElement.querySelector('.js-input-min');
    let maxInput = priceFilterElement.querySelector('.js-input-max');
    let globalMin = priceFilterElement.dataset.globalMin;
    let globalMax = priceFilterElement.dataset.globalMax;
    let inputMin = minInput ? minInput.value : globalMin;
    let inputMax = maxInput ? maxInput.value : globalMax;

    priceFilterElement.dataset.currentMin = inputMin;
    priceFilterElement.dataset.currentMax = inputMax;

    delete filterValues['minPrice'];
    delete filterValues['maxPrice'];

    if (inputMin != globalMin) {
      filterValues['minPrice'] = inputMin;
    }

    if (inputMax != globalMax) {
      filterValues['maxPrice'] = inputMax;
    }
  }

  requestCatalogByFilters();
  closeCatalogFiltersDrawer();
}

function syncCatalogFiltersDrawerState() {
  if (!isCatalogFiltersMobileLayout()) {
    closeCatalogFiltersDrawer();
  }
}

function ensureShipsAllModelsOption(shipsFilterElement) {
  if (!shipsFilterElement) {
    return false;
  }

  let insertElement = shipsFilterElement.querySelector('.js-ajax-insert');

  if (!insertElement || insertElement.querySelector('.js-ships-filter-all')) {
    return false;
  }

  let allRow = document.createElement('div');
  allRow.className = 'row js-apply-filter js-ships-filter-all active';
  allRow.dataset.valueId = '';
  allRow.textContent = 'Все модели';
  insertElement.prepend(allRow);

  return true;
}

function initCatalogShipsFilter() {
  let shipsFilterElement = document.querySelector('.js-filter-item[data-filter-name="ships"]');

  if (!shipsFilterElement || shipsFilterElement.classList.contains('js-hidden')) {
    return false;
  }

  ensureShipsAllModelsOption(shipsFilterElement);

  let filterTitle = shipsFilterElement.querySelector('.js-filter-title');
  let defaultValue = filterTitle?.dataset.defaultValue || 'Все модели';
  let valueItems = shipsFilterElement.querySelectorAll('.js-apply-filter');
  let allRow = shipsFilterElement.querySelector('.js-ships-filter-all');
  ...
  if (filterTitle) {
    filterTitle.innerHTML = defaultValue;
  }

  shipsFilterElement.classList.remove('selected');
  delete filterValues['ships'];

  return true;
}

function syncCatalogShipsFilterAfterAjax() {
  let shipsFilterElement = document.querySelector('.js-filter-item[data-filter-name="ships"]');
  ...
  let selectedShipId = filterValues['ships'];

  if (!selectedShipId) {
    return initCatalogShipsFilter();
  }

  let filterTitle = shipsFilterElement.querySelector('.js-filter-title');
  let valueItems = shipsFilterElement.querySelectorAll('.js-apply-filter');
  let selectedRow = shipsFilterElement.querySelector(
    `.js-apply-filter[data-value-id="${selectedShipId}"]`
  );
  ...
  if (selectedRow) {
    ...
    if (filterTitle) {
      filterTitle.innerHTML = selectedRow.innerHTML;
    }

    shipsFilterElement.classList.add('selected');
  } else {
    delete filterValues['ships'];
    initCatalogShipsFilter();
  }
...
function resetCatalogFilters() {
  ...
  filterItems.forEach(item => {
    ...
    let listValues = item.querySelectorAll('.js-apply-filter');
    listValues.forEach(value => {
      value.classList.remove('active');
    });

    let filterTitle = item.querySelector('.js-filter-title');
    let filterToggle = item.querySelector('.js-filter-toggle');

    if (filterTitle && filterTitle.dataset.defaultValue) {
      filterTitle.innerHTML = filterTitle.dataset.defaultValue;
    }
    ...
    if (item.dataset.filterType === 'price') {
      let globalMin = item.dataset.globalMin;
      let globalMax = item.dataset.globalMax;
      let minInput = item.querySelector('.js-input-min');
      let maxInput = item.querySelector('.js-input-max');
      let minRange = item.querySelector('.js-range-min');
      let maxRange = item.querySelector('.js-range-max');
      let progress = item.querySelector('.js-price-progress');

      item.dataset.currentMin = globalMin;
      item.dataset.currentMax = globalMax;

      if (minInput) minInput.value = globalMin;
      if (maxInput) maxInput.value = globalMax;
      if (minRange) minRange.value = globalMin;
      if (maxRange) maxRange.value = globalMax;
      if (progress) {
...
function setInputMasks() {
  ...
  if (priceInputs) {
    let catalogPriceInputs = document.querySelectorAll('.homepage .catalog-filters .js-price-input');
    let maskedPriceInputs = [...priceInputs].filter((input) => {
      return ![...catalogPriceInputs].includes(input);
    });

    if (maskedPriceInputs.length) {
      new MaskInput(maskedPriceInputs, { mask: "#######" })
    }
  }
...
function bindEvents() {
  ...
  window.addEventListener('resize', syncCatalogFiltersDrawerState);
  ...
  addOnClickListeners('.js-catalog-filters-open', openCatalogFiltersDrawer);
  addOnClickListeners('.js-catalog-filters-close', closeCatalogFiltersDrawer);
  addOnClickListeners('.js-catalog-filters-apply', applyCatalogFiltersFromDrawer);
  addOnClickListeners('.js-catalog-filters-reset', resetCatalogFilters);
...
function applyFilter(event) {
  ...
  if (filterType == 'list') {    
    ...
    let isShipsFilter = filterName === 'ships';
    let isDesktopInlineList = (
      filterName === 'duration' || filterName === 'interval'
    );

    if (isShipsFilter && !targetButton) {
      toggleFilterItem(event);
      return;
    }

    if (isDesktopInlineList && targetButton) {
      let valueItems = filterItem.querySelectorAll('.js-apply-filter');
      let targetIsActive = targetButton.classList.contains('active');
      ...
      if (targetIsActive) {
        ...
        targetButton.classList.add('active');
        filterValues[filterName] = targetButton.dataset.valueId;
    ...
    if (isShipsFilter && targetButton) {
      let targetIsActive = targetButton.classList.contains('active');
      let isAllModelsOption = targetButton.classList.contains('js-ships-filter-all')
        || targetButton.dataset.valueId === '';
      let valueItems = filterItem.querySelectorAll('.js-apply-filter');
      let defaultValue = filterTitle.dataset.defaultValue || 'Все модели';

      if (isAllModelsOption) {
        valueItems.forEach(el => {
          el.classList.remove('active');
        });
        targetButton.classList.add('active');
      ...
      targetButton.classList.add('active');
      filterTitle.innerHTML = targetButton.innerHTML;
      filterValues[filterName] = targetButton.dataset.valueId;
      filterItem.classList.add('selected');
      closeFilterItems();
      requestCatalogByFilters();
...
function requestCatalogByFilters() {
  ...
  .then(function (response) {
    ...
    initNzMoskvaWidgets();
    initTeplohodWidgets();
    syncCatalogShipsFilterAfterAjax();
    addOnClickListeners('.js-apply-filter', applyFilter);
  })
